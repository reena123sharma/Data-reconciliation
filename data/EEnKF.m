...
%% assume that we are getting correct values of voltage
 if(i==1)
  ...
  for j=1:Np
   ...
   A = -1/29 - 1000000*0.6*(0.004186/625)*(1 - exp(-0.00000036*(9.955+0.0091935*x_elecp(2))/(0.6*0.004186)));
   B = 3.6*21*(V_electro(i-1) -1.482)/625 - 3.6*(0.0091935/625)*(x_elecp(1) - 14.5)*exp(-0.00000036*(9.955 + 0.0091935*x_elec(2))/(0.6*0.004186));
  
   G = exp(A*Ts);
   Hf = (G -1)*B*inv(A);
   ET_pf(j) = G*ET_p(j) + Hf*EI_elecp(j) + Q*randn();
  ...
  ET(i) = (1/Np)*sum(ET_pf); 
  ET1(i) = ET(i);
  EI_elec(i) = (1/Np)*sum(EI_elecpf); 
  x_elec = [ET(i);EI_elec(i)];
  
  P = zeros(2,2);
  for j=1:Np
   e = [ET_pf(j);EI_elecpf(j)] - [ET(i);EI_elec(i)];
   P = P + e*e';
  end
  P = P./(Np-1);
