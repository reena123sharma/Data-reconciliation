...
%%--------------DATA-----------%%
load('Ts_30.mat');
% t = xlsread('data_central','Sheet2','A3:A573');
% G = xlsread('data_central','Sheet2','B3:B573');
% V_solar = xlsread('data_central','Sheet2','C3:C573');
% I_solar = xlsread('data_central','Sheet2','D3:D573');
% P_solar = V_solar.*I_solar;
% 
% V_electro = xlsread('data_central','Sheet2','E3:E573');
% I_electro = xlsread('data_central','Sheet2','F3:F573');
% T = xlsread('data_central','Sheet2','G3:G573');
% P_electro = V_electro.*I_electro/1000;
% 
% m = length(Time);
% 
% U_FC = xlsread('data_central','Sheet2','H3:H573');
% I_FC = xlsread('data_central','Sheet2','I3:I573');
...
%% assume that we are getting correct values of voltage
 if(i==1)
  EI_elec(i) = MI_electro(1);
  ET(i) = MT(1);
  EP_elec(i) = EI_elec(i)*V_electro(i)/1000;

 else 
  i   
  A = -1/29 - 1000000*0.6*(0.004186/625)*(1 - exp(-0.00000036*(9.955+0.0091935*EI_elec(i-1))/(0.6*0.004186)));
  B = 3.6*21*(V_electro(i-1) -1.482)/625 - 3.6*(0.0091935/625)*(ET(i-1) - 14.5)*exp(-0.00000036*(9.955 + 0.0091935*EI_elec(i-1))/(0.6*0.004186));
  C = UrevUI(1) + rUI(1)*(EI_elec(i-1)/2.5) + (polyval(sUI,ET(i-1))/(polyval(tUI,ET(i-1))*(EI_elec(i-1)/2.5) + 1))*(2*tUI(1)*ET(i-1) + tUI(2))*(EI_elec(i-1)/2.5) + sUI(1)*log(polyval(tUI,ET(i-1))*(EI_elec(i-1)/2.5) + 1);
  D = polyval(rUI,ET(i-1))/2.5 + polyval(sUI,ET(i-1))*((polyval(tUI,ET(i-1)))/(polyval(tUI,ET(i-1))*EI_elec(i-1) + 2.5));

  A_aug = [A,B ; -inv(D)*C*A, -inv(D)*C*B];
  phi = exp(A_aug.*Ts); 
  Ty = [1; -inv(D)*C];
  %%Predication Step
   Gf = exp(A*Ts);
   H = (Gf -1)*B*inv(A);
   ET(i) = Gf*ET(i-1) + H*EI_elec(i-1);
   UI = @(x)polyval(UrevUI,ET(i)) + polyval(rUI,ET(i))*x + polyval(sUI,ET(i))*log(polyval(tUI,ET(i))*x + 1) - V_electro(i-1);
   EI_elec(i) = (fsolve(UI, 45))*2.5;
  ...
  %%update
   ...
   ET(i) = x_elec(1);  
   %ET(i) = ET(i) + L(1)*(MT(i) - ET(i));
   EI_elec(i) = fsolve(UI, 40)*2.5;
   P = (eye(2) - L*H)*P;
   EP_elec(i) = EI_elec(i)*V_electro(i)/1000;
