...
Ts_T = 6/3600; %%sampling time for temperature
mRR = Ts_T/Ts;
H = [1,0;0,1];
H_f = [0,1];
P = [0.01 , 0 ; 0, 0.01];
...
%% assume that we are getting correct values of voltage
 if(i==1)
  ...
  if(mod((i-1),mRR)~=0)          %minor sampling time
   ...
   %%update
   for j=1:Np
    x_elecp = [ET_p(j);EI_elecp(j)] + L_f*( [MI_electro(i)] +  mvnrnd(0,R_f) - H_f*[ET_p(j);EI_elecp(j)]) ;
    ET_p(j) = x_elecp(1);
  ...
  elseif(mod((i-1),mRR)==0)
   ...
   %%update
   for j=1:Np
    x_elecp = [ET_p(j);EI_elecp(j)] + L*( [MT(iT);MI_electro(i)] +  mvnrnd([0;0],R)' - H*[ET_p(j);EI_elecp(j)]) ;
    ET_p(j) = x_elecp(1);
    UI = @(x)polyval(UrevUI,ET_p(j)) + polyval(rUI,ET_p(j))*x + polyval(sUI,ET_p(j))*log(polyval(tUI,ET_p(j))*x + 1) - V_electro(i);
    EI_elecp(j) = (fzero(UI, 45))*2.5;
   end
