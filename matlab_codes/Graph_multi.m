%load('Ts_2_6.mat','T','I_electro','t')
load('ts_6.mat','T','t')
t_T = t;
clear t;

load('Ts_2.mat','I_electro','t')
%Even the measurements are avilable at every 6s, estimation has been done for 2s. 

load('Multi_RPF_2_6_res.mat','ET','EI_elec','m','MT', 'MI_electro');
ET_RPF = ET;
EI_RPF = EI_elec;
clear ET;
clear EI_elec;

load('Multi_ENKF_2_6_res1.mat','ET','EI_elec');
ET_EnKF = ET;
EI_EnKF = EI_elec;
clear ET;
clear EI_elec;

load('multiEKF_2_6_res.mat','ET','EI_elec')
ET_EKF = ET;
EI_EKF = EI_elec;
clear ET;
clear EI_elec;
...
ax1 = subplot(2,1,1);
plot(t_T, MT, ':go','MarkerIndices',1:10:length(t_T),'MarkerSize',3,'MarkerFaceColor','g','LineWidth',0.01)
hold on;
