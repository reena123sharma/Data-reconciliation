...
Np = 20
H = [1,0;0,1];
H_f = [1,0];% temp. measurment current not
P = [0.1 , 0 ; 0, 0.1];
S = chol(P);
Q = 0.05;
R = [4,0;0,9];
...
% Initial Particles
  ...
  elseif (i>=m_start && i <=m_end )         %shutdown time
  i
   v1=ET_p;
   v2= EI_elecp;
%Prediction step%  
  parfor j=1:Np
   ...
   UI = @(x)polyval(UrevUI,v1(j)) + polyval(rUI,v1(j))*x + polyval(sUI,v1(j))*log(polyval(tUI,v1(j))*x + 1) - V_electro(i-1);
   EI_elecp(j) = (fzero(UI, 45))*2.5;
   x_elecp(j,:) = [ET_p(j),EI_elecp(j)];
  
% Importance weight calculation
   wp1(j) = (1/(det(R_f))^0.5)*exp( (-([MT(i)] - H_f*x_elecp(j,:)')'*inv(R_f)*([MT(i)] - H_f*x_elecp(j,:)'))/2 ); % MT TEMPERATURE
...
%State estimate
 ...
 %%random sampling
  w = abs(mu - MT(i));%%width temp .measur
  if w>1
      w = 1;
  end    
  
  for j=1:200    %%
    ET_pf(j) = (mu -0.4*w) + rand(1,1)*2*(0.4*w);
...
%State estimate
 ...
 %%random sampling
  ...
  for j=1:200    %%
    ET_pf(j) = (mu -0.3*w) + rand(1,1)*2*(0.3*w);
    prob(j) = pdf(ET_pf(j),Np,wp,ET_p,A_r,h);
  end 
