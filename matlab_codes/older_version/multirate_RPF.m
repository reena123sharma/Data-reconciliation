...
Ts =2/3600;%%minorsampling time
Ts_T = 0.1/60;%%minorsampling time
mRR = Ts_T/Ts;
Np = 20
H = [1,0;0,1];
H_f = [0,1];
...
% Initial Particles
 for j=1:Np
  ...
  elseif(mod((i-1),mRR)~=0)          %minor sampling time
  i
...
ET_modi = zeros(m_T,1);
ET1_modi = zeros(m_T,1);
filled = floor(m/mRR) + 2;
iT = 0;
for i = 1:m
 if (i==1)
   ET_modi(i) = ET(i);
...
figure(4)
plot(t_T,T,'-b',t_T,MT,'r*',t,ET,'-g')
xlabel('time(hrs)')
ylabel('temp.(C)')
legend('true','measured','estimated')
title('Electrolyzer')
print -color -depsc Temp.eps
