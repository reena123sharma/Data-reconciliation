...
Ts =6/3600;%%minorsampling time(in hrs)
T_start = 1  %in hr
T_end = 2    % in hr
...
% Importance weight calculation
   wp1(j) = (1/(det(R_f))^0.5)*exp( (-([MT(i)] - H_f*x_elecp(j,:)')'*inv(R_f)*([MT(i)] - H_f*x_elecp(j,:)'))/2 );
  end
   mu = mean(x_elecp(:,1));
  ...
  %Prediction step%  
   ...
   %Importance weight calculation
    wp1(j) = (1/(det(R))^0.5)*exp( (-([MT(i);MI_electro(i)] - H*x_elecp(j,:)')'*inv(R)*([MT(iT);MI_electro(i)] - H*x_elecp(j,:)'))/2 );
   end
   mu = mean(x_elecp(:,1));
   S_r = zeros(1,1);  %%currnet prediction covariance 
   for j=1:Np
...
%%Regularized particle filter resampling
 ...
 %%random sampling
  ...
  for j=1:5*Np    %%
    ET_pf(j) = (mu -0.6*w) + rand(1,1)*2*(0.6*w);
